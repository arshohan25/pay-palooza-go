...
const typeConfig: Record<string, { icon: typeof Gift; gradient: string; badge: string; action: string }> = {
  coupon: { icon: BadgePercent, gradient: "from-emerald-500/15 to-teal-500/5", badge: "bg-emerald-500/15 text-emerald-700 dark:text-emerald-400", action: "Claim" },
  loan: { icon: Landmark, gradient: "from-blue-500/15 to-indigo-500/5", badge: "bg-blue-500/15 text-blue-700 dark:text-blue-400", action: "Apply" },
  gift_card: { icon: Gift, gradient: "from-purple-500/15 to-pink-500/5", badge: "bg-purple-500/15 text-purple-700 dark:text-purple-400", action: "Claim" },
  offer: { icon: Ticket, gradient: "from-amber-500/15 to-orange-500/5", badge: "bg-amber-500/15 text-amber-700 dark:text-amber-400", action: "Claim" },
};
...
export default function AiRewardBanner({ rewards, onClaim, onApply }: Props) {
  ...
  const handleAction = async (reward: AiReward) => {
    ...
    if (ok) toast.success("Reward claimed! 🎉");
  };
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-1.5 px-0.5">
        ...
        <span className="text-[9px] px-1.5 py-px rounded-full bg-primary/10 text-primary font-medium">Smart</span>
      </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-1 text-[10px] text-muted-foreground">
                  ...
                  <span>{daysLeft(reward.expires_at)}d left</span>
                </div>
              </div>
              <button
