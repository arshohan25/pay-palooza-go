...
import { Search, ChevronDown, ChevronUp, Loader2, RefreshCw, Download } from "lucide-react";
...
export default function AdminChargebackHistory() {
  ...
  const loadData = useCallback(async () => {
    ...
    // Fetch audit logs for chargebacks
    ...
    // Map audit entries by target user + approximate time match
    const auditByEntity: Record<string, AuditEntry> = {};
    (audits ?? []).forEach(a => {
      // Match by entity_id (target user) — use the reference_txn_id from details if available
      const key = a.entity_id;
      if (key && !auditByEntity[key]) {
        auditByEntity[key] = a as AuditEntry;
      }
    });

    // Better matching: map audit to transaction by matching details
    const auditByTxn: Record<string, AuditEntry> = {};
    rows.forEach(txn => {
      // Find matching audit log by entity_id + closest timestamp
    ...
    // Resolve profiles for both target users and admin actors
    const allIds = [
      ...targetUserIds,
  ...
  const exportCsv = () => {
    const headers = ["Short ID", "Date", "User Name", "User Phone", "Amount", "Reason", "Admin", "Reference", "Balance After"];
                  ...
                  return (
                    <Fragment key={txn.id}>
                      ...
                      >
                        ...
                        <td className="px-4 py-3 font-semibold text-destructive">-৳{txn.amount?.toLocaleString()}</td>
                        ...
                        {isExpanded && (
                          <tr>
                            <td colSpan={7} className="p-0">
                              ...
                              >
                                <div className="bg-muted/30 px-6 py-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
                                  <div>
