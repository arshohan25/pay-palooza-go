...
export function AdminApprovalQueue() {
  ...
  const create = async () => { const { error } = await supabase.from("admin_approval_requests" as any).insert({ ...form, action_type: form.action_type.toLowerCase().replaceAll(" ", "_"), payload: { source: "approval_queue" } } as any); if (error) toast.error("Failed to create request"); else { toast.success("Approval request created"); setOpen(false); load(); } };
