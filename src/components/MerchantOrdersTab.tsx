...
const STATUS_META: Record<string, { labelKey: TranslationKey; color: string; icon: React.ElementType }> = {
  processing: { labelKey: "moStatusProcessing", color: "#FF9800", icon: Clock },
  confirmed: { labelKey: "moStatusConfirmed", color: "#9C27B0", icon: CheckCircle2 },
  partially_shipped: { labelKey: "moStatusPartiallyShipped", color: "#0288D1", icon: Truck },
  shipped: { labelKey: "moStatusShipped", color: "#2196F3", icon: Truck },
  out_for_delivery: { labelKey: "moStatusOutForDelivery", color: "#FF5722", icon: Package },
  delivered: { labelKey: "moStatusDelivered", color: "#43A047", icon: CircleCheck },
  cancelled: { labelKey: "moStatusCancelled", color: "#9E9E9E", icon: X },
};
...
const MerchantOrdersTab = ({ merchantId }: Props) => {
  ...
  const advanceStatus = async (order: MerchantOrder) => {
    ...
    const { error } = await (supabase as any)
      ...
      .eq("id", order.id);
    if (error) toast({ title: t("moUpdateFailed"), variant: "destructive" });
    else { toast({ title: t("moOrderTo").replace("{status}", t(STATUS_META[next]?.labelKey)) }); load(); }
  };
  ...
  if (orders.length === 0) return (
    <div className="text-center py-16 space-y-3">
      ...
      <p className="text-[15px] font-bold text-foreground">{t("moNoOrders")}</p>
      <p className="text-[13px] text-muted-foreground">{t("moOrdersAppearHere")}</p>
    </div>
  );
        ...
        {STATUS_FLOW.map(s => {
          ...
          return (
            <div key={s} className="shrink-0 flex items-center gap-1.5 bg-card border border-border/60 rounded-xl px-3 py-1.5">
              ...
              <span className="text-[11px] font-semibold text-foreground">{t(meta.labelKey)}</span>
              <span className="text-[11px] font-bold" style={{ color: meta.color }}>{fmt(count)}</span>
            </div>
          );
        })}
      </div>
            ...
            <button className="w-full p-3.5 flex items-center gap-3 text-left"
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10.5px] text-muted-foreground mt-0.5">
                  {new Date(order.created_at).toLocaleDateString(lang === "bn" ? "bn-BD" : "en-US", { month: "short", day: "numeric" })} · {fmt(items.length)} {items.length !== 1 ? t("moItems") : t("moItem")} · ৳{fmt(Number(order.total))}
                </p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <span className="text-[9px] font-bold text-white px-2 py-0.5 rounded-full"
                  style={{ background: meta.color }}>{t(meta.labelKey)}</span>
                ...
                {items.map((item: any, j: number) => (
                  <div key={j} className="flex items-center gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-muted-foreground">{t("moQty")} {fmt(item.qty)} · ৳{fmt(Number(item.price))}</p>
                    </div>
                  </div>
                ))}
