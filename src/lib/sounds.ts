...
function playRingChime(ctx: AudioContext) {
  const now = ctx.currentTime;
  // Pleasant two-tone ring pattern
  playTone(587.33, now, 0.15, 0.14, ctx);         // D5
  playTone(783.99, now + 0.15, 0.15, 0.12, ctx);  // G5
  playTone(587.33, now + 0.5, 0.15, 0.14, ctx);   // D5
  playTone(783.99, now + 0.65, 0.15, 0.12, ctx);  // G5
}
...
function playRingbackBeep(ctx: AudioContext) {
  const now = ctx.currentTime;
  playTone(440, now, 0.4, 0.06, ctx);             // A4 — subtle long beep
  playTone(440, now + 0.6, 0.4, 0.06, ctx);       // A4
}
...
export function playRingbackTone() {
  ...
  if (!ctx) return;
  if (ctx.state === "suspended") ctx.resume();
  playRingbackBeep(ctx);
  ringbackInterval = setInterval(() => {
    const c = getAudioContext();
    if (c) playRingbackBeep(c);
  }, 3000);
}
