...
const T = {
  en: {
    ...
    enterPin: "Enter PIN",
    trustedDevice: "Trusted device — enter PIN to continue",
    trustedVerified: "Trusted Device",
    ...
    securePayments: "Secure Payments", quickSetup: "Quick Setup",
  },
  bn: {
    ...
    enterPin: "পিন দিন",
    trustedDevice: "বিশ্বস্ত ডিভাইস — পিন দিন",
    trustedVerified: "বিশ্বস্ত ডিভাইস",
    ...
    securePayments: "নিরাপদ পেমেন্ট", quickSetup: "দ্রুত সেটআপ",
  },
} as const;
...
// ─── PIN Dots (UPI style — large, glowing) ────────────────────────────────────
          ...
          <motion.div
            ...
            animate={{
              ...
              backgroundColor: error
                ? "hsl(var(--destructive))"
                : filled
                ? dark ? "white" : "hsl(var(--primary))"
                : "transparent",
            }}
            transition={{ type: "spring", stiffness: 500, damping: 22 }}
            className={`w-4 h-4 rounded-full border-2 transition-all ${
              error && filled
                ? "border-destructive shadow-[0_0_12px_hsl(var(--destructive)/0.5)]"
                : filled
                ? dark
                  ? "border-white shadow-[0_0_16px_rgba(255,255,255,0.4)]"
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      <motion.div
        ...
        className="absolute top-1/3 left-1/2 -translate-x-1/2 w-40 h-40 rounded-full bg-white/3 blur-2xl"
      />
    </div>
  );
}
...
export default function AuthPage({ onAuthenticated }: AuthPageProps) {
  ...
  // ── Full-screen gradient pages (login_pin uses gradient bg like BHIM) ──────
  const isFullGradientPage = mode === "login_pin";
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col bg-background overflow-hidden">
        ...
        {mode === "landing" && (
          ...
          >
            ...
            <div className="relative gradient-hero flex-1 flex flex-col items-center justify-center px-6 overflow-hidden">
              ...
              <div className="relative flex flex-col items-center gap-6 text-white">
                ...
                <motion.div
                  ...
                  className="relative"
                >
                  <div className="w-24 h-24 rounded-[28px] bg-white/15 backdrop-blur-sm border border-white/25 flex items-center justify-center text-5xl font-black shadow-float">
                    ৳
                  </div>
                  {/* Pulsing rings */}
                  {[1.3, 1.6, 1.9].map((s, i) => (
                ...
                >
                  ...
                  <p className="text-sm text-white/60 mt-2 font-medium">{t.taglineBn}</p>
                </motion.div>
                ...
                >
                  {[
                    { icon: Zap, label: t.instantTransfer },
                    { icon: Shield, label: t.bankSecurity },
                    { icon: Fingerprint, label: t.securePayments },
                    { icon: Star, label: t.zeroFees },
                  ].map(({ icon: Icon, label }) => (
            ...
            >
              ...
              {isNewUser ? (
                <button
                  className="w-full text-sm text-center py-1 text-muted-foreground"
                  onClick={() => {
                    setPhone(""); setOtp(""); setPin(""); setConfirmPin(""); setConfirmStage(false);
                ...
                <button
                  className="w-full text-sm text-center py-1 text-muted-foreground"
                  onClick={() => {
                    localStorage.removeItem(REGISTERED_KEY); localStorage.removeItem(PIN_KEY); localStorage.removeItem(DEVICE_KEY);
      ...
      {/* ══════════════════════ LOGIN PIN — full gradient screen (BHIM style) ══════════════════════ */}
      <AnimatePresence mode="wait">
        {mode === "login_pin" && (
          <motion.div
            key="login_pin"
          ...
          >
            ...
            <div className="relative z-10 px-4 pt-4 flex items-center justify-between"
              ...
              <button onClick={handleBack}
                className="w-10 h-10 rounded-full bg-white/12 border border-white/15 flex items-center justify-center text-white active:scale-90 transition-transform">
                <ChevronLeft size={18} />
              </button>
      ...
      {/* ══════════════════════ OTHER STEPS (phone, otp, pin-set, name) ══════════════════════ */}
                  ...
                  >
                    ...
                    {(mode === "register_pin" || mode === "forgot_pin") && (() => {
                      ...
                      return (
                        <div className="space-y-6">
                          <div className="space-y-1">
                            <AnimatePresence mode="wait">
                              <motion.div key={confirmStage ? "confirm" : "set"}
                    ...
                    {mode === "register_name" && (
                      <div className="space-y-6">
                        ...
                        <motion.button
                          whileTap={{ scale: 0.97 }}
                          className="w-full h-14 gradient-hero text-white font-bold text-[15px] rounded-2xl shadow-glow flex items-center justify-center gap-2"
