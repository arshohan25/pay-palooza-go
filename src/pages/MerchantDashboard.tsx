          ...
          <AnimatePresence mode="wait">
            {step === "details" ? (
              <motion.div key="details" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                ...
                <Button onClick={goToPin} disabled={quotaRemaining <= 0 || dailyRemaining <= 0} className="w-full h-12 rounded-xl text-sm font-bold gradient-primary text-primary-foreground">
                  {quotaRemaining <= 0 ? "Quota Exhausted" : dailyRemaining <= 0 ? "Limit Reached" : "Continue → Enter PIN"}
                </Button>
              </motion.div>
            ) : step === "pin" ? (
