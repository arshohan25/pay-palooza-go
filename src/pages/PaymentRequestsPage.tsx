...
const PaymentRequestsPage = () => {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: linksData, error: le }, { data: paysData }] = await Promise.all([
      ...
      supabase.from("payment_link_payments").select("id,link_id,payer_id,amount,status,created_at,transaction_id,refunded_at,refund_reason,refunded_amount").eq("payee_id", user.id).order("created_at", { ascending: false }).limit(500),
    ]);
