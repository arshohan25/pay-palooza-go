...
`nid_front` → `nid_back` → `nid_details` → `**additional_info**`→`selfie`   → `review` → `submitted`
...
- Add state for: `occupation`, `gender`, `monthlyIncome`, `address (should be grab from NID back side)`, `maritalStatus`
...
- Options for each field:
  ...
  - **Occupation**: Student, Business, Government Job, Private Job, Freelancer, Homemaker, Retired, Probashi, Farmer, House Wife, Other
